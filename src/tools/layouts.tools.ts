...
const ESTRUTURA_PARECER = {
  ...
  secoes: [
    ...
    },
    {
      numero: 5,
      titulo: 'ASSINATURA',
      conteudo: '"É o parecer, s.m.j." | Fortaleza, [DATA POR EXTENSO]. | Parágrafos centralizados — 2 pareceristas fixos: Prof. Fellipe Guerra (Contador e Advogado Tributarista, CRC/CE nº 21.074 | OAB/CE nº 49.759) e Prof. Marcos Lima (Contador e Cientista de Dados, CRC/CE nº 23.224). Formato: linha de assinatura + nome bold + cargo + registro. Vedada qualquer alteração.',
    },
  ],
  instrucoes_gerais: [
...
const REGRAS_DOCX = {
  ...
  paragrafos: {
    espaco_antes_pt: 0,
    espaco_depois_pt: 4,
    recuo_primeira_linha_cm: 2.0,
    regra: 'Parágrafos de corpo com recuo de primeira linha 2,0 cm (pf.first_line_indent = Cm(2.0)). Separados por space_after 4pt. Títulos de seção (I., II., II.1 etc.) sem recuo (first_indent=0). Seções com space_before 14pt.',
    implementacao: 'pf.line_spacing = Pt(18) | pf.space_before = Pt(0) | pf.space_after = Pt(4) | pf.first_line_indent = Cm(2.0)',
  },
  titulo: {
    alinhamento: 'centralizado',
    bold: true,
    tamanho_pt: 13,
    subtitulo_proibido: 'NÃO adicionar "PARECER Nº XX/XXXX" abaixo do título principal',
  },
  negrito: {
    regra: 'Negrito APENAS em títulos de seção (I. RELATÓRIO, II. FUNDAMENTAÇÃO, II.1, etc.) e na EMENTA. PROIBIDO negrito em texto corrido do corpo.',
    permitido: ['Títulos de seção e subseção', 'EMENTA (toda em maiúsculas e negrito)', 'Rótulos do cabeçalho (CONSULENTE, DATA, ASSUNTO, LEGISLAÇÃO)'],
    proibido: ['Qualquer palavra em negrito dentro de parágrafos do Relatório, Fundamentação, Conclusão, Exemplos'],
  },
  citacoes: {
    formato: 'Bloco de citação recuado 2,5 cm à esquerda, sem recuo de primeira linha, itálico, tamanho 11pt',
    implementacao: 'pf.left_indent = Cm(2.5) | pf.first_line_indent = Cm(0) | run.italic = True | run.font.size = Pt(11)',
  },
  cor_texto_principal: '#1A1A2E (Guerra dark navy) — NÃO usar preto puro #000000 nem #1A1A1A',
  cores_permitidas: ['azul-escuro (texto) #1A1A2E', 'branco #FFFFFF', 'cinza bordas #808080', 'cinza header tabela #EEEEEE', 'cinza zebra #F0F0F0', 'cinza médio #D0D0D0'],
  cores_proibidas: [
    'preto puro #000000 (usar #1A1A2E)',
    'azul claro (qualquer tom)',
    'vermelho (qualquer tom)',
    'verde (qualquer tom)',
    'amarelo (reservado para XLSX)',
    'qualquer outra cor não listada acima',
  ],
  cabecalho_tabela: {
    shade_rotulo: '#EEEEEE',
    bordas: 'auto (color="auto", sz=4)',
    larguras_dxa: 'col0=2200, col1=6826',
    alinhamento_rotulo: 'LEFT (não CENTER)',
  },
  tabelas: 'Dados sempre em tabela — nunca em parágrafos corridos. Tabelas de conteúdo usam bordas #808080.',
  titulos_secao: 'Alinhamento justificado. Bold. Sem recuo de primeira linha.',
  implementacao_python: {
    ...
    codigo: [
      ...
      '# Título de seção (sem recuo):',
      'pf.first_line_indent = Cm(0)',
      '# Citação (recuada, itálico):',
  ...
  erros_comuns: [
    ...
    'Texto alinhado à esquerda — proibido (usar JUSTIFY)',
    'Recuo de primeira linha menor que 2cm — proibido no corpo',
    'Negrito em texto corrido — proibido (apenas títulos e EMENTA)',
    'Citação sem itálico e sem recuo — proibido',
    'Cor #1A1A1A — usar #1A1A2E',
    'Shade de cabeçalho #C8C8C8 — usar #EEEEEE',
    'Dados em parágrafos corridos sem tabela — proibido',
    'Qualquer cor além de preto/branco/cinza (#1A1A2E, #EEEEEE, #F0F0F0, #D0D0D0, #808080, #FFFFFF) — proibido',
  ],
};
...
const CHECKLIST_DOCX = [
  'Documento gerado a partir do template Guerra (paper timbrado herdado do modelo)?',
  'Espaçamento 1,5 (line_spacing=Pt(18)) em todos os parágrafos?',
  'space_before=0pt e space_after=4pt em parágrafos normais?',
  'Recuo de primeira linha 2,0 cm em parágrafos de corpo?',
  'Títulos de seção (I., II., II.1) sem recuo de primeira linha e com bold?',
  'Negrito APENAS em títulos de seção e EMENTA — zero negrito no corpo do texto?',
  'Citações com itálico e left_indent=2,5 cm (sem recuo de primeira linha)?',
  'Texto justificado (exceto título principal que é centralizado)?',
  'Título principal centralizado sem subtítulo "PARECER Nº XX/XXXX"?',
  'Cabeçalho: labels alinhados à esquerda, shade #EEEEEE, bordas auto?',
  'Cor do texto #1A1A2E (não #000000 nem #1A1A1A)?',
  'Dados em tabelas (não parágrafos corridos)?',
  'Tabelas apenas em preto/branco/cinza (sem azul, vermelho, verde)?',
  'Travessões (—) removidos do texto corrido?',
  'Palavras de linguagem IA revisadas e substituídas?',
  'Assinatura: apenas Fellipe Guerra e Marcos Lima — Mathaus Pordeus removido?',
];
